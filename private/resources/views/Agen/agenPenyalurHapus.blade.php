				...
				<div class="col-sm-10">
					<nav aria-label="breadcrumb">
						<ol class="breadcrumb">
							...
							<li class="breadcrumb-item"><a href="{{url('dataPenyalur')}}">Data Penyalur</a></li>
							<li class="breadcrumb-item active" aria-current="page">Hapus Penyalur</li>
						</ol>
					</nav>
					@if(session('sukses'))
					...
					<div class="card">
						...
						<div class="card-body text-center">
							<h5>Apakah Akan Deaktivasi</h5>
							 {!! Form::open(array('route' => ['penyalur.editHapus',$penyalur],'method'=>'POST', 'enctype' => 'multipart/form-data' )) !!}
								...
								<table class="table table-bordered table-hover">
									<thead>
										<tr>
											...
											<th scope="col">Alamat Penyalur</th>
										</tr>
									</thead>
									<tbody>
										<tr>
											<td>{{$penyalur->idAgen}}</td>
											<td>{{$penyalur->namaPenyalur}}</td>
											<td>{{$penyalur->alamatPenyalur}}</td>
										</tr>
									</tbody>
								</table>
								<a href="{{url('dataPenyalur')}}" class="btn btn-outline-info btn-sm">
